    ...
    toGOOD(): IArtifact {
        ...
        for (const roll of this.substats.split) {
            if (!initialValues.has(roll.fightProp)) {
                initialValues.set(roll.fightProp, roll.isPercent ? Math.round(roll.getMultipliedValue() * 10) / 10 : Math.round(roll.getMultipliedValue()));
            }
        }

        const formatValue = (substat: StatProperty) => substat.isPercent ? Math.round(substat.getMultipliedValue() * 10) / 10 : Math.round(substat.getMultipliedValue());
